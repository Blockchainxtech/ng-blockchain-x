...
const bxHelper = new BlockchainxHelper();
...
export const chainInfo = (chainId: string) => {
    if (bxHelper.isHex(chainId) == true) {
        // Hexa value 
        const chainInDecimal = bxHelper.hexToDecimal(chainId);
        // @ts-ignore
        return chains[Number(chainInDecimal)];
    } else {
        // @ts-ignore
...
export const getAddNetworkParam = (chainId: string) => {
    const chainDetail = chainInfo(chainId);
    const params: any = {
        chainId: bxHelper.decimalToHex(chainDetail.chainId),
        chainName: chainDetail.name,
        rpcUrls: chainDetail.rpc,
        nativeCurrency: chainDetail.nativeCurrency
    }
    if (chainDetail.explorers != undefined && chainDetail.explorers.length > 0) {
        params.blockExplorerUrls = [ chainDetail.explorers[0].url ]
    }
    return {
